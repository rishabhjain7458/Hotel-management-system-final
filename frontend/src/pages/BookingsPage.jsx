...
const BookingsPage = () => {
    ...
    const fetchAvailableRooms = async () => {
        try {
            ...
            const data = await response.json();
            const roomsReadyForCheckIn = data.data.rooms.filter(room => room.readyForCheckIn);
    ...
    const deleteBooking = async (bookingId) => {
        try {
            const response = await fetch(`http://localhost:2020/bookings/${bookingId}`, {
                method: 'DELETE',
            });
    ...
    return (
        <div className="bookings-container">
            ...
            <button onClick={deleteAllBookings} className="btn-delete-all"  style={{backgroundColor:"#AD8B3A"}}>Delete All Bookings</button>
            {bookings.length === 0 ? (
                ...
                <div className="bookings-grid">
                    {bookings.map((booking) => (
                        <div key={booking._id} className="booking-card">
                            ...
                            <p><strong>Checkout Date:</strong> {new Date(booking.checkoutDate).toLocaleDateString()}</p>
                            <p><strong>Payment Status:</strong> {booking.paymentStatus}</p>
                            <button onClick={() => deleteBooking(booking._id)} className="btn-delete" style={{backgroundColor:"#AD8B3A"}}>Delete</button>
                        </div>
                    ))}
                </div>
            )}
